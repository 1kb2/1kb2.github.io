document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderPage(target, param = null) {
        ...
        switch(target) {
            ...
            case 'certs':
                loadAllCerts();
                activeFileLabel.innerText = '~/certs/';
                break;
            case 'blog':
                loadBlogList();
        ...
        if (!initialLoad && window.goatcounter && window.goatcounter.count) {
            let trackPath;
            if (target === 'post-detail') trackPath = `/blog/${param}`;
            else if (target === 'note-detail') trackPath = `/notes/${param}`;
            else if (target === 'about') trackPath = '/';
            else trackPath = `/${target}`;
            window.goatcounter.count({ path: trackPath });
        }
        initialLoad = false;
    ...
    function loadNeofetch() {
        ...
        bufferContent.innerHTML = `
            ...
            <div class="social-links">
                ...
                <a href="https://profile.hackthebox.com/profile/019ddeb2-9ec3-719f-89bc-a80ce8f23c55" target="_blank">[ HackTheBox ]</a>
            ...
            <p style="color: #666;">// <strong>COMMANDS:</strong> :projects, :certs, :blog, :notes, :about</p>
        `;
    }
    ...
    async function loadAllCerts() {
        try {
            ...
            let html = `<h1># Directory: ~/certs</h1>`;
                    ...
                    if (isPlanned) {
                        ...
                        footerHtml = `
                            <div style="display:flex; justify-content:space-between; gap: 10px; margin-top: 10px; font-size:0.7rem; color:#888; border-top: 1px dashed #333; padding-top: 5px;">
                                ...
                                <span style="color: var(--active-green);">[ DOES NOT EXPIRE ]</span>
                            </div>`;
                    } else {
                        footerHtml = `
            ...
            const updateTimers = () => {
                ...
                data.forEach(cert => {
                    ...
                    if (noExpiry) {
                        elBadge.innerHTML = `<span class="active-badge">ACTIVE</span>`;
                        elTimer.innerText = "DOES NOT EXPIRE";
                        return;
                    }
    ...
    function handleCommand(cmd) {
        ...
        else if (cleanCmd === 'certs') window.navigate('certs');
